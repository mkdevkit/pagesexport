...
export default function NewArticlePage() {
  ...
  const checkDirectoryExists = async (directory: string): Promise<boolean> => {
    try {
      const response = await fetch(`/api/check-directory?directory=${encodeURIComponent(directory)}`)
      const result = await response.json()
  ...
  const handleDirectoryChange = async (value: string) => {
    ...
    if (value) {
      const exists = await checkDirectoryExists(value)
      if (exists) {
        setDirectoryError(`目录 "${value}" 已存在，请使用其他目录名`)
      } else {
        setDirectoryError('')
      }
    } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-md px-3 py-2 ${directoryError ? 'border-red-500' : 'border-gray-300'}`}
                  required
                />
                <p className="text-xs text-gray-500 mt-1">文章的英文目录名,用于存放文件</p>
                {directoryError && <p className="text-sm text-red-500 mt-1">{directoryError}</p>}
              </div>
